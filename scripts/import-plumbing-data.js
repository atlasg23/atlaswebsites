...
async function importPlumbingData() {
  try {
    console.log('Starting plumbing data import...\n');

    // Read the mobile filtered plumbing CSV (the correct one)
    let csvPath = path.join(process.cwd(), 'data', 'mobile_filtered_plumbing_data.csv');

    if (!fs.existsSync(csvPath)) {
      console.error('mobile_filtered_plumbing_data.csv not found!');
      console.log('Looking for alternative CSV files...');

      // List available CSV files
      const dataDir = path.join(process.cwd(), 'data');
      const files = fs.readdirSync(dataDir).filter(f => f.endsWith('.csv'));
      console.log('Available CSV files:', files);
      return;
    }
    ...
    console.log(`\nPrepared ${businesses.length} records for import`);

    // Clear existing data first
    console.log('Clearing existing data in businesses table...');
